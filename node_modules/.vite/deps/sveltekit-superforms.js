...
var root_5 = add_locations(template(`<div> </div>`), SuperDebug.filename, [[707, 5]]);
var root_8 = add_locations(template(`<div class="super-debug--promise-loading">Loading data...</div>`), SuperDebug.filename, [[724, 70]]);
var root_10 = add_locations(template(`<span class="super-debug--promise-rejected">Rejected:</span> <!>`, 1), SuperDebug.filename, [[727, 23]]);
var root_12 = add_locations(template(`<button type="button" class="super-debug--collapse"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path fill="currentColor" d="M4.08 11.92L12 4l7.92 7.92l-1.42 1.41l-5.5-5.5V22h-2V7.83l-5.5 5.5l-1.42-1.41M12 4h10V2H2v2h10Z"></path></svg></button>`), SuperDebug.filename, [
  [
    734,
    3,
    [[739, 4, [[745, 6]]]]
...
var root_2 = add_locations(template(`<div class="super-debug" dir="ltr"><div><div class="super-debug--label"> </div> <div class="super-debug--right-status"><button type="button" class="super-debug--copy"><!></button> <!></div></div> <pre class="super-debug--pre"><code class="super-debug--code"><!></code></pre> <!></div>`), SuperDebug.filename, [
  [
    657,
    1,
    [
...
function SuperDebug($$anchor, $$props) {
  ...
  legacy_pre_effect(
    () => (deep_read_state(data()), deep_read_state(raw())),
    () => {
      store_unsub(set(debugData, assertStore(data(), raw()) ? data() : readable(data())), "$debugData", $$subscriptions);
    }
  );
  legacy_pre_effect_reset();
  init();
  ...
  if_block(node, () => !get(styleInit), ($$anchor2) => {
    var style = root_1();
  ...
  if_block(node_1, display, ($$anchor2) => {
    var div = root_2();
    var div_1 = child(div);
    ...
    var node_3 = sibling(sibling(button, true));
    if_block(node_3, status, ($$anchor3) => {
      var div_4 = root_5();
      var text_2 = child(div_4);
      template_effect(() => {
        toggle_class(div_4, "super-debug--info", $page().status < 200);
        toggle_class(div_4, "super-debug--success", $page().status >= 200 && $page().status < 300);
        toggle_class(div_4, "super-debug--redirect", $page().status >= 300 && $page().status < 400);
        toggle_class(div_4, "super-debug--error", $page().status >= 400);
    ...
    slot(node_4, default_slot($$props), {}, ($$anchor3) => {
        ...
        ($$anchor4) => {
          ...
          await_block(
            ...
            ($$anchor5, result) => {
              ...
              html(node_7, () => syntaxHighlight(assertStore(result, raw()) ? get_store_value(result) : result), false, false);
              append($$anchor5, fragment_3);
            },
            ($$anchor5, error) => {
              var fragment_4 = root_10();
              var span_1 = first_child(fragment_4);
              var node_8 = sibling(sibling(span_1, true));
              html(node_8, () => syntaxHighlight(error), false, false);
    ...
    var node_10 = sibling(sibling(pre, true));
    if_block(node_10, collapsible, ($$anchor3) => {
      var button_1 = root_12();
      var svg_2 = child(button_1);
      var path_4 = child(svg_2);
      template_effect(() => toggle_class(svg_2, "super-debug--rotated", collapsed()));
    ...
    template_effect(() => {
      ...
      set_class(div_1, `super-debug--status ${(label() === "" ? "super-debug--absolute super-debug--inset-x-0 super-debug--top-0" : "") ?? ""}`);
